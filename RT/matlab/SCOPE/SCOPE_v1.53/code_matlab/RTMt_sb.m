...
% 1.2 radiance by leaf layers Hv and by soil Hs
for j = 1:nl
    Hcsu2   = Hcsu3(:,:,j);
    Hcsu(j) = mean(Hcsu2'*lidf);
end
...
if obsdir
    ...
    for j = 1:nl
        piLo2(j)= iLAI*epsc*mean((Hcsu3(:,:,j).*abs(fo))'*lidf,1)*gap.Pso(j);% directional   emitted     radiation by sunlit leaves
    end
