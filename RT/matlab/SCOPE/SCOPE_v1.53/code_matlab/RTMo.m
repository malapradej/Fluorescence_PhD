...
%% Section for calculating spectral absorption by the soil
% J.Malaprade
% includes all wavelengths

% shaded soil
Rndifsoil_  = 0.001*Emin_(nl+1, :)'.*epss; %?)'
Rnhs_       = Rndifsoil_;

% sunlit soil
Rndirsoil_  = 0.001*Esun_.*epss;
Rnus_       = Rndifsoil_ + Rndirsoil_;

Rnstot_     = Rnhs_ + Rnus_;

%% Section for calculating spectral total canopy outgoing radiance (upward + downward)
% J.Malaprade
% includes all wavelenghts

canopy_out_rad = 0.001*(Emin_(end,:)' + Eout_);

%% Section for calculating spectral absorption including PAR to Fs range per leaf layer
% J. Malaprade
% the wavelength range is from 400 to 850 nm which included PAR and Fs
% ranges.

% sunlit leaves
minwl       = min(spectral.wlPAR);  % wavelengths
maxwl       = max(spectral.wlF);
minwli      = find(spectral.wlP == minwl); % indices
maxwli      = find(spectral.wlP == maxwl);
IparFs      = [minwli:maxwli]';     % indices of wl's including PAR and Fs
wlPARFs     = [minwl:maxwl]';       % wl range including PAR and Fs
canopy.IparFs     = IparFs;
nwlPARFs    = length(IparFs);       % number of wavelengths
Pndir_      = zeros(nli, nlazi, nwlPARFs);
Pndifs_     = zeros(nl, nwlPARFs);
Pnuc_       = zeros(nli, nlazi, nl, nwlPARFs);
%Pnu1d_      = zeros(nl, nwlPARFs);

Pnsun_      = 0.001 * e2phot(wlPARFs*1E-9, Esun_(IparFs).*epsc(IparFs)); % absorbed solar radiation in umol m-2 s-1

for i = 1:nli
    for j = 1:nlazi
        Pndir_(i,j,:) = fs(i,j)*Pnsun_(:); % net absorbed per leaf area (W m-2) per leaf angle per wl
    end
end

for l = 1:nl
    Pndifs_(l,:) = .001 *(e2phot(wlPARFs*1E-9, Rndif_(l,IparFs)'))'; % net absorbed radiation as photons
    for i = 1:nli
        for j = 1:nlazi
            for k = 1:nwlPARFs
                Pnuc_(i,j,l,k) = Pndir_(i,j,k) + Pndifs_(l,k); % total irradiance on sunlit leaves 
            end
        end
    end
end

[Pnu1d_ ] = meanleaf(canopy,Pnuc_,'angles_spectral');

% shaded leaves
Pnhc_       = Pndifs_; % [nl, nwlPARFs]

% Proportional to sunlit and shaded leaf
Pn1d_       = zeros(nl, nwlPARFs);

for l = 1:nl
    Pn1d_(l,:) = (1 - Ps(l))*Pnhc_(l,:) + Ps(l)*Pnu1d_(l,:); % spectrally absorbed radiation per layer in umol m-2 s-1
end
%% Section for calculating spectral absorption for full canopy and PAR and SIF wavelengths
% J. Malaprade
% a different approach to the one above per layer.

% sunlit leaves
Rnsun_          = 0.001 * Esun_(IparFs).*epsc(IparFs);      % [nwl] absorbed solar radiation in W m-2

Rndir_          = zeros(nli, nlazi, nwlPARFs);              % [13, 36, nwl] initialize
for i = 1:nli
    for j = 1:nlazi
        for k = 1:nwlPARFs
            Rndir_(i,j,k) = fs(i, j) .* Rnsun_(k);          % absorbed solar flux by leaves
        end
    end
end

Rnuc_           = zeros(nli, nlazi, nl, nwlPARFs);          % [nl, 13, 36, nwl] initialize
for l = 1:nl
    for i = 1:nli
        for j = 1:nlazi
            for k = 1:nwlPARFs
                Rnuc_(i,j,l,k) = Rndir_(i,j,k) + 0.001*Rndif_(l,k);% total fluxes on sunlit leaves
            end
        end
    end
end

Rnu_            = meanleaf(canopy, Rnuc_, 'angles_layers_spectral',Ps);
Rnu_            = reshape(Rnu_,[1, nwlPARFs]);   % [nwl] flatten 4D array

% shaded leaves
Rnh_            = 0.001*Rndif_(:, IparFs);        % [nl, nwl] net radiation diffuse shaded leaves

% total absorbed over canopy per wavelenght
Fc = (1 - Ps(1:end-1))'/nl;
Rnctot_          = LAI*(Fc*Rnh_ + Rnu_)';     % [nwl] net radiation absorbed per wavelength in W m-2
...
if options.calc_vert_profiles   
    ...
    profiles.Pn1d_      = Pn1d_;                                     % [nl, nwlPAR]      mean spectral absorbed PAR per layer
else
    profiles = struct;
end
...
rad.canopy_out_rad = canopy_out_rad; % [2162x1 double] total canopy outgoing radiance upward and downward (W m-2)

rad.Rnhs    = Rnhs;         % [1 double]        net radiation (W m-2) of shaded soil
rad.Rnus    = Rnus;         % [1 double]        net radiation (W m-2) of sunlit soil
rad.Rnstot_ = Rnstot_;      % [2162x1 double]   net spectral radiation (W m-2 um-1) of soil JM.
rad.Rnctot_ = Rnctot_;      % [451x1 double]    net spectral absorption (W m-2 um-1) of canopy JM.
