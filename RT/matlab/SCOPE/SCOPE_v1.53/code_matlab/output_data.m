...
% added by JM: total net spectral radiation absorbed by soil
fidtss          = [Output_dir,'soil_spectral_absorption.dat'];
dlmwrite(fidtss, rad.Rnstot_, 'delimiter','\t', 'precision','%9.5f');

% added by JM: total net spectral radiation absorbed by canopy
fidtcs          = [Output_dir,'canopy_spectral_absorption.dat'];
dlmwrite(fidtcs, rad.Rnctot_, 'delimiter','\t', 'precision','%9.5f');
...
% JM: saves leaf spectral albedo file
fidlfal         = [Output_dir,'leaf_spectral_albedo.dat'];
leaf_albedo     = leafopt.refl + leafopt.tran;
dlmwrite(fidlfal, leaf_albedo, 'delimiter','\t', 'precision','%3.5f');

% JM: saves canopy spectral outgoing radiation file
fidcaal         = [Output_dir,'canopy_spectral_outgoing_rad.dat'];
dlmwrite(fidcaal, rad.canopy_out_rad, 'delimiter','\t', 'precision','%3.8f');

% JM: saves canopy spectral outgoing fluorescence file
fidcsfl         = [Output_dir,'canopy_spectral_outgoing_SIF.dat'];
Fs_out          = (rad.Fhem_ + rad.Fhem_bottom_)/1000;
dlmwrite(fidcsfl, Fs_out, 'delimiter','\t', 'precision','%3.8f');

% JM: saves sum of spectral fluorescence matrices file including
% fluorescence efficiencies in canopy
fidcsfm         = [Output_dir,'leaf_spectral_fluorescence_mat.dat'];
eta             = mean(profiles.etah) + mean2(profiles.etau); % average efficiency in canopy
Fs_out          = sum(leafopt.MbI+leafopt.MfI+(leafopt.MbII+leafopt.MfII)...
    *eta, 2) / 2;
dlmwrite(fidcsfm, Fs_out, 'delimiter','\t', 'precision','%3.8f');
...
if options.calc_vert_profiles
    ...
    fidplS          = [Output_dir,'layer_aPAR_Fs_spectral.dat'];
    dlmwrite(fidplS,1E6*profiles.Pn1d_,'delimiter','\t', ...
        'precision','%9.5f')
    ...
    if options.calc_ebal
        ...
        if options.calc_fluor
            ...
            % JM: saves the spectral Fs per layer
            fidfllemwlf     = [Output_dir,'layer_fluorescenceEm_spectral.dat'];
            dlmwrite(fidfllemwlf,profiles.fluorescenceEmwlF,'delimiter','\t', ...
                'precision','%9.5f');
