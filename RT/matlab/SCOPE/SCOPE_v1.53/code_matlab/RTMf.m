...
LoF_                = zeros(nwlfo,2);
Fhem_               = zeros(nwlfo,2);
Fhem_bottom_        = zeros(nwlfo,2); % added by JM
...
for PS = 2:-1:1
    ...
    for i = 1 : nwlfo
        ...
        % JM: based on Emin_(end,:) in RTMo.m. The scattering at BOC contributing to canopy albedo.
        Fhem_bottom_(i,PS) = (G1(end)+rinf*G2(end))/(1-rinf^2); % JM: the hemispherical Fs at the bottom of canopy per PS
    end
...
rad.Fhem_bottom_ = Fhem_bottom_(:,1) + Fhem_bottom_(:,2); % JM: the total hemispherical Fs at the BOC

profiles.fluorescence   = Fiprofile(:,1) + Fiprofile(:,2);
profiles.fluorescenceEm = zeros(nl+1,1);
profiles.fluorescenceEmwlF = zeros(nl+1,nwlfo); % JM: spectral Fs per layer
for i = 1:nl+1
    profiles.fluorescenceEm(i) = 0.001 * Sint(sum(piLem_(:,i,:),3),spectral.wlF)';
    
    % added by J.Malaprade
    % the vertical profile of spectral fluorescence in W m-2 nm-1
    profiles.fluorescenceEmwlF(i,:) = 0.001 * sum(piLem_(:,i,:),3)';
end
